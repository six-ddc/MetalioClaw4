...
inline const FontTier& FontTierAt(uint8_t idx) {
    // 阅读三档 小/中/大 = 20/30/40（40 号为普惠体常用字子集，line_height 取自生成字体）。
    static const FontTier kTiers[3] = {
        {&font_puhui_20_4, 25},
        {&font_puhui_30_4, 38},
        {&font_puhui_40_4, 49},
    };
...
inline PageMetrics BuildMetrics(const ReaderSettings& s) {
    ...
    uint16_t em = lv_font_get_glyph_width(ft.font, 0x4E00, 0);
    if (em == 0) em = static_cast<uint16_t>(ft.line_height);
    m.indent_w = static_cast<int16_t>(em);
    m.heading_font = hf.font;
    m.heading_line_height = hf.line_height;
